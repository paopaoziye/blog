...
>**格式**：`[说明符] [声明符]`，其中**数据类型**大部分和C一致，主要多出了`bool`和`long long`类型
{%list%}
...
{%list%}
基本与C语言一致，Cpp常用空指针为nullptr，避免使用NULL和0
{%endlist%}
...
{%endlist%}
{%right%}
const可以修饰类型和对象，前者为底层const，后者为顶层const
{%endright%}
```cpp
const char *a //a为一个指针常量
char *const b //b为一个常量指针
...
{%endright%}
>上述情况下，**常量引用**实际上指向**编译器创建**的**引用类型**的**临时变量**，见《C++ primer》p55
{%warning%}
...
{%endlist%}
{%right%}
有些初始值的类型会被编译器改写后再赋予变量，可以自行修补，如const auto a和auto &b 
{%endright%}
