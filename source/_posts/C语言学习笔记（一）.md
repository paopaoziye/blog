...
#### 1.1C语言程序框架
**①预处理命令**
>`#include <xx.h>`：将**对应文件内容**逐字写到**对应位置**，**常用的库**有`stdio`和`stdlib`
...
{%endlist%}
{%warning%}
仅规定了长度上长整型≥整型≥短整型，且short int至少占16位，long int至少占32位，他们能表示的范围是不同的
{%endwarning%}
{%right%}
...
{%list%}
也没有规定浮点类型的长度，仅规定长度上long double≥doubl≥float
{%endlist%}
{%warning%}
