...
#### 2.1格式
**①源文件**
>**概述**：由**返回类型**、**函数名**、**参数列表**和**函数主体**组成
{%list%}
函数可以没有返回值，这是返回类型为void
{%endlist%}
{%warning%}
函数中的形参是实参的一个副本，修改形参不会影响实参，如果想要修改实参，需要传递实参的地址
{%endwarning%}
```
...
```
>**`return`语句**：用于**返回结果**并表明此处是函数的**退出点**，可以出现在**函数主体**的**任意位置**
{%list%}
如果函数主体的最后一条语句不是return语句，则该语句之后为函数的隐式退出点
{%endlist%}
{%warning%}
